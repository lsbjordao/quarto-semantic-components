...
[`v0.12.0`]{.badge type="accent" font="mono"}
```

**Output:** [**Stable**]{.badge type="success" appearance="outline" size="md"} [`v0.12.0`]{.badge type="accent" font="mono"}
...
The `+` character has no special meaning in `tree`; it is shown above only as ordinary content. Branches are inferred solely from nested children. Use `expanded=`, `open=`, or `collapsed=` on a span when a branch needs a specific initial state. `line-color-light` and `line-color-dark` can tune connector contrast by theme.
...
```markdown
:::: {.article accent="left" accent-color="success"}
## Release package [v0.12.0]{.badge type="success" appearance="solid" size="xs"}
...
| Format | Behavior |
|---|---|
| **HTML** | Full styling, light/dark color overrides, generic/file trees, SVG Git DAG, native progress/meter, keyboard/abbreviation semantics, and article collapse/expand |
